...
<title>@yield('title','Siswa - Al-Mustofa')</title>
...
.sidebar{width:260px;height:100vh;background:#7c3aed;color:#ffffff;position:sticky;top:0;flex-shrink:0;display:flex;flex-direction:column;}
...
<small style="opacity:.8;">Siswa</small>
</div>
<nav class="nav flex-column gap-1 sidebar-menu" id="sidebar-menu">
<a href="{{ route('siswa.nilai.index') }}" class="nav-link d-flex align-items-center gap-2 px-3 py-2 rounded {{ request()->routeIs('siswa.nilai.*') ? 'active' : '' }}" onclick="closeSidebarIfMobile()"><i class="bi bi-clipboard-data"></i> Nilai Saya</a>
<a href="{{ route('siswa.hafalan.index') }}" class="nav-link d-flex align-items-center gap-2 px-3 py-2 rounded {{ request()->routeIs('siswa.hafalan.*') ? 'active' : '' }}" onclick="closeSidebarIfMobile()"><i class="bi bi-book-half"></i> Hafalan</a>
<a href="{{ route('siswa.karakter.index') }}" class="nav-link d-flex align-items-center gap-2 px-3 py-2 rounded {{ request()->routeIs('siswa.karakter.*') ? 'active' : '' }}" onclick="closeSidebarIfMobile()"><i class="bi bi-heart"></i> Karakter</a>
</nav>
...
<nav class="d-md-none bg-white border-bottom p-2 d-flex align-items-center"><button class="btn btn-outline-secondary btn-sm" onclick="openSidebar()"><i class="bi bi-list"></i></button><span class="ms-2 fw-semibold">Siswa</span></nav>
