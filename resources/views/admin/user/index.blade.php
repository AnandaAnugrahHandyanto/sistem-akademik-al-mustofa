...
<tbody>
@forelse($data as $r)
<tr><td class="ps-3 fw-medium">{{ $r->username }}</td><td>{{ $r->name }}</td><td><span class="badge bg-warning text-dark">{{ $r->role }}</span></td><td class="small text-muted">{{ $r->email }}</td>
<td class="text-end pe-3"><a href="{{ route('admin.user.edit',$r) }}" class="btn btn-sm btn-outline-primary me-1"><i class="bi bi-pencil"></i></a><form method="POST" action="{{ route('admin.user.destroy',$r) }}" class="d-inline">@csrf @method('DELETE')<button class="btn btn-sm btn-outline-danger" onclick="return confirm('hapus?')"><i class="bi bi-trash"></i></button></form></td></tr>
