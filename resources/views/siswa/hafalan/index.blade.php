...
<tbody>
@forelse($data as $r)
<tr><td>{{ $r->jenis_hafalan }}</td><td>{{ $r->surat }}</td><td><div class="progress" style="height:16px"><div class="progress-bar" style="width:{{ $r->progress }}%">{{ $r->progress }}%</div></div></td><td>{{ $r->tanggal }}</td><td>@if($r->audio)<a href="{{ Storage::url($r->audio) }}" target="_blank" class="btn btn-sm btn-outline-primary">Play</a>@else<span class="text-muted">-</span>@endif</td></tr>
