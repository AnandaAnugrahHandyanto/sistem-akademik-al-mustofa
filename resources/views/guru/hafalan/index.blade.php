...
<thead><tr><th class="ps-3 text-nowrap" style="min-width:150px">Siswa</th><th class="text-nowrap" style="min-width:120px">Surat</th><th class="text-nowrap text-center" style="min-width:80px">Ayat</th><th style="min-width:140px" class="text-nowrap">Progress</th><th class="text-nowrap" style="min-width:80px">Status</th><th class="text-nowrap" style="min-width:80px">Audio</th><th class="text-nowrap" style="min-width:110px">Tanggal</th><th class="text-end pe-3 text-nowrap" style="min-width:110px">Aksi</th></tr></thead>
<tbody>
@forelse($data as $r)
<tr><td class="ps-3 fw-medium text-nowrap">{{ $r->siswa->nama_lengkap }} <small class="text-muted">({{ $r->siswa->kelas }})</small></td><td>{{ $r->surat }}<br><small class="badge bg-light text-dark border">{{ $r->jenis_hafalan }}</small></td><td class="text-center text-nowrap">@if($r->ayat_mulai && $r->ayat_selesai){{ $r->ayat_mulai }}-{{ $r->ayat_selesai }}@elseif($r->ayat_mulai){{ $r->ayat_mulai }}@else<span class="text-muted">-</span>@endif</td><td><div class="progress" style="height:20px;"><div class="progress-bar {{ $r->progress <= 50 ? 'bg-danger' : ($r->progress <= 80 ? 'bg-warning text-dark' : 'bg-success') }}" role="progressbar" style="width: {{ $r->progress }}%;" aria-valuenow="{{ $r->progress }}" aria-valuemin="0" aria-valuemax="100">{{ $r->progress }}%</div></div></td><td>@php $st=$r->status ?? 'baru'; @endphp<span class="badge {{ $st=='lulus'?'bg-success':($st=='ulang'?'bg-warning text-dark':'bg-secondary') }}">{{ ucfirst($st) }}</span></td><td>@if($r->audio)<a href="{{ Storage::url($r->audio) }}" target="_blank" class="btn btn-sm btn-outline-success"><i class="bi bi-play-circle me-1"></i>Play</a>@else<span class="text-muted">-</span>@endif</td><td class="small text-muted">{{ $r->tanggal }}</td>
...
@empty<tr><td colspan="8" class="text-center text-muted py-4">Belum ada data hafalan</td></tr>@endforelse
</tbody></table>
</div></div>
